...
const hotkey_on_key_down = event => {
	event.preventDefault();
	const char = event.which || event.keyCode;

	if(keys.length === 0) { // new hotkey input
		keys = [];
		keysRendered = [];
	}

	const i = keys.indexOf(char);

	if (i < 0) {
		keys.push(char);
		keysRendered.push(char);
	}

	hotkey_render_output();
}

const hotkey_on_key_up = event => {
	event.preventDefault();
	const char = event.which || event.keyCode;
...
const hotkey_render_output = () => {
	let value = [];

	for(const key of keysRendered) {
		value.push(keyMapping[key] || String.fromCharCode(key));
	}

	document.getElementById("settings_hotkey").value = value.join(" + ");
}
...
const getSettings = async () => {
	return new Promise(async (resolve, reject) => {
		...
		boundAsync.getSettings().then(settings => {
